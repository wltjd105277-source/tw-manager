...
export default function Home() {
  ...
  const setupDefaultQuests = async () => {
    if (!confirm('현재 리스트를 지우고 지성 님의 고정 숙제로 자동 세팅할까요?')) return
  ...
  const toggleQuest = async (id: number, is_done: boolean, category: string) => {
    ...
    if (category.startsWith('계정')) {
      await supabase.from('tw_quests').update({ is_done: nextStatus }).eq('category', category)
    ...
    <main className="min-h-screen bg-[#f8fafc] pb-24 font-sans text-slate-800">
      <header className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-100 p-5">
        <div className="flex justify-between items-center mb-5">
          ...
          <div className="flex gap-2">
            {isAdmin && (
              <button onClick={setupDefaultQuests} className="text-[10px] bg-green-500 text-white px-3 py-1 rounded-full font-bold animate-bounce">
                🚀 고정 숙제 세팅
              </button>
            )}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {characters.map(char => (
            <button key={char} onClick={() => setActiveChar(char)} className={`px-5 py-2 rounded-2xl font-bold text-sm transition-all whitespace-nowrap ${activeChar === char ? 'bg-blue-600 text-white shadow-md' : 'bg-slate-100 text-slate-400'}`}>{char}</button>
          ))}
        </div>
      </header>
      ...
      <div className="p-5 max-w-md mx-auto space-y-8">
        {[
          ...
          <div key={section.id}>
            <h3 className="text-xs font-black text-slate-400 mb-4 flex items-center gap-2 tracking-widest uppercase">
              <div className="w-1 h-4 bg-blue-500 rounded-full" /> {section.title}
            </h3>
            <div className="space-y-3">
              {getQuests(section.id).map(q => (
                <div key={q.id} onClick={() => toggleQuest(q.id, q.is_done, q.category)} className={`flex justify-between p-5 rounded-3xl border-2 transition-all cursor-pointer ${q.is_done ? 'bg-slate-50 border-transparent opacity-50' : 'bg-white border-white shadow-sm'}`}>
